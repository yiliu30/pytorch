...
class TestQuantizePT2EX86Inductor(X86InductorQuantTestCase):
    ...
    def test_set_module_name(self):
        ...
        quantizer.set_module_name("sub", xiq.get_default_x86_inductor_quantization_config())
        node_occurrence = {
    ...
    def test_set_module_name_with_underscores(self) -> None:
        ...
        # Set global to no quantization and then default config for a specific submodule.
        quantizer = X86InductorQuantizer()
        quantizer.set_module_name("foo_bar", xiq.get_default_x86_inductor_quantization_config())
        ...
        for n in m.graph.nodes:
            if n.op == "call_function" and n.target == torch.ops.aten.linear.default:
                ...
                if count == 0:
                    # for foo_bar.
                    self.assertEqual(weight_observer_node.op, "call_module", f"The op of linear({count})'s weight_observer_node is {weight_observer_node.op} instead call_module")
                    ...
                    self.assertNotEqual(weight_observer_node.op, "call_module", f"The op of linear({count})'s weight_observer_node is {weight_observer_node.op} instead call_module")
                count += 1
